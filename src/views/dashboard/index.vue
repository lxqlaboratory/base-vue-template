  ...
  <div style="height: calc(100vh - 34px);width: 100%;">
    <el-container style="height: calc(100vh - 34px);">
      ...
      <el-main style="height: calc(100vh - 34px);padding:0px;">
        <baidu-map :center="center" :zoom="zoom" style="height: 100%;width: 100%;" @ready="handler"
                   @click="getClickInfo">
          ...
          <bm-marker v-for="marker of markers" :position="{lng: marker.lng, lat: marker.lat}" @click="infoWindowOpen"
                     title="杨培林">
    ...
    <el-dialog title="轨迹回放" width="80vw;" :visible.sync="trackPlaybackVisible">
      <el-input v-model="trackPlaybackStartTime" type="date" size="small" placeholder="请输入开始时间"
                suffix-icon="el-icon-date"></el-input>
      <el-input v-model="trackPlaybackEndTime" type="date" size="small" placeholder="请输入结束时间"
                suffix-icon="el-icon-date"></el-input>
...
<script>
  ...
  import {
    getTreeVehicleFormList,
    getVehiclePositionFromList,
    getSelectedVehiclePosition
  } from '@/api/vehicle-list-index'
    ...
    methods: {
      ...
      getChecked() {
        ...
        this.checkedNodes.forEach(item => {
          if (item.plateNum) {
            this.plateNumList2.push(item.plateNum)
          }
        })
      ...
      trackPlaybackDraw() {
        getVehiclePositionFromList(this.trackPlaybackStartTime, this.trackPlaybackEndTime).then(response => {
          ...
          if (this.vehiclePositionFromList.style() > 0) {
            ...
            this.trackPlaybackStartPoint = {
              lng: this.vehiclePositionFromList[0].lng,
              lat: this.vehiclePositionFromList[0].lat
            }
            this.trackPlaybackEndPoint == {
              lng: this.vehiclePositionFromList[this.vehiclePositionFromList.style() - 1].lng,
              lat: this.vehiclePositionFromList[this.vehiclePositionFromList.style() - 1].lat
            }
